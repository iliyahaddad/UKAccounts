...
public partial class ProfitLossView : Window
{
    ...
    private async Task LoadReportAsync()
    {
        ...
        _report = await _reportService.GetProfitLossAsync(Guid.Empty, fromDate, toDate);
        ...
        ProfitLossDataGrid.ItemsSource = rows;
    }

    private void ExportPdf_Click(object sender, RoutedEventArgs e)
    {
        if (_report == null) return;
        var pdf = PdfExporter.ExportProfitLoss(_report);
        SaveFile("PDF files (*.pdf)|*.pdf", "profit_and_loss.pdf", pdf);
    }

    private void ExportCsv_Click(object sender, RoutedEventArgs e)
    {
        if (_report == null) return;
        var csv = CsvExporter.ExportProfitLoss(_report);
        SaveFile("CSV files (*.csv)|*.csv", "profit_and_loss.csv", csv);
    }
